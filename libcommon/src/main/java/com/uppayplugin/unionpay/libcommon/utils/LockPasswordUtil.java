...
public class LockPasswordUtil {
	private static final String TAG = LockPasswordUtil.class.getSimpleName();
	...
	private static class PasswordFileObserver extends FileObserver {
		...
		public void onEvent(int event, String path) {
			if (LOCK_PASSWORD_FILE.equals(path)) {
				Logger.d(TAG, "lock password file changed");
	...
	public void saveLockPassword(String password, int quality, boolean isFallback) {
		...
		try {
			...
			// provider
			Logger.e(TAG, "Unable to save lock pattern to " + sLockPasswordFilename);
	...
	public byte[] passwordToHash(String password) {
		...
		try {
			...
			Logger.w(TAG, "Failed to encode string because of missing algorithm: " + algo);
		}
		return hashed;
	}
	...
	private String getSalt() {
		...
		if (salt == 0) {
			try {
				...
				Logger.v(TAG, "Initialized lock password salt");
			} catch (NoSuchAlgorithmException e) {
