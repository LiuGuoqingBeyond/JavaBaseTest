...
public class SortUtils {
    ...
    public static SortUtils getInstance(Context context){
        mContext = context;
        if (instance == null){
    ...
    public List<String> listSort(List<String> list) {
        ...
        for (int i = 0; i < array.length - 1; i++) {
            for (int j = i + 1; j < array.length; j++) {
                if (singleSort(array[i].trim(), array[j].trim()) == 1) {
                    String temp = array[i];
                    array[i] = array[j];
                    array[j] = temp;
                }
     ...
     * @return
     */
    public int singleSort(String one, String two) {
     ...
     * @return
     */
    private int chineseCompare(String one, String two, int i) {
     ...
     * @return
     */
    private int intCompare(int subLeft, int subRight) {
        if (subLeft > subRight) {
            return 1;
